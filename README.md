...
2. **目的地を検索** — 店名・施設名・住所を入れて「検索」を押す
3. **駐車場をおすすめする（トグル）**
   - **ON** — 「周辺の駐車場を検索」を押すと候補がスコア順に出る。カードか地図のピンで 1 つ選ぶ
   - **OFF** — 駐車場をはさまず目的地まで直行
4. **ルート確定** — 下部のボタンで Google マップを開く

駐車場の検索は明示的にボタンを押したときだけ実行します。絞り込みを変えている途中で
何度も Overpass を叩かないようにするためです。

## 地点検索

店名で引けないことが多かったため、次の多段構えにしています。

1. **Photon と Nominatim を同時に引く**
   Photon（komoot）は POI 名の検索とタイポに強く、Nominatim は住所に強い。
   結果は名前と座標で重複を除いてから統合する
2. **現在地による優先**
   出発地が設定されていれば、両サービスに位置バイアスを渡し（Photon は `lat`/`lon`、
   Nominatim は `viewbox`）、結果も現在地から近い順に並べ替えて距離を表示する
3. **語を落として再検索**
   全角空白を含めて正規化し、全語で 0 件なら長い語から順に試す。
   「台湾鍋　民生炒飯」→「民生炒飯」。OSM には屋号だけが登録されていることが多いため
4. **Overpass の名称検索**
   ジオコーダが拾えなくても、OSM に `name` があれば見つかることがある。
   語の選択和（`民生炒飯|台湾鍋`）で現在地から 30km 以内を引く
5. **それでも駄目なら逃げ道を出す**
   「地図から指定する」（タップした地点を目的地にする。地名は逆ジオコーディングで補う）と
   「Google マップで探す」を提示して、行き止まりにしない
